...
class Customer extends Admin
{
    ...
    public function balance_log()
    {
        ...
        $list = M('balance_log b')->join('customer c', 'c.id=b.customer_id')->where($map)->field('b.*,c.username,c.headimg')->order("b.create_time desc")->paginate(30);
        $this->assign('_list', $list);
        return view();
    }
    ...
    public function integral_log()
    {
        ...
        $list = M('integral_log')->where($map)->order("create_time desc")->paginate(30);
        $this->assign('_list', $list);
        return view();
    }
    ...
    public function hz_price()
    {
        ...
        $cates = ProductModel::getProducts($map, $customer['grade_id'], $customer['f_id']);
    ...
    public function hz_price_edit()
    {
        if (request()->isPost()) {
            ...
            $product = ProductModel::getProduct($map, $customer['grade_id']);
