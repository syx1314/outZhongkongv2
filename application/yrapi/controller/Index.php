...
class Index extends Home
{
    ...
    public function recharge()
    {
        ...
        if ($reod = M('porder')->where(['out_trade_num' => $out_trade_num, 'customer_id' => $this->customer['id']])->find()) {
            return djson(1, '已经存在相同商户订单号的订单');
        }
        $res = PorderModel::createOrder($mobile, $product_id, $this->customer['id'], Client::CLIENT_API, 'api下单', $out_trade_num);
        if ($res['errno'] != 0) {
    ...
    public function product()
    {
        $map['p.is_del'] = 0;
        $map['p.added'] = 1;
        $lists = ProductModel::getProducts($map, $this->customer['grade_id'], $this->customer['f_id']);
