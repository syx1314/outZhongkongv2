...
class Member extends Model
{
    ...
    public function up_pwd($id, $ypwd, $npwd)
    {
        if ($user = $this->get(['id' => $id])) {
            if ($user['password'] == dyr_encrypt($ypwd)) {
                return $this->update(['id' => $id, 'password' => dyr_encrypt($npwd)]);
            } else {
                return false;
            }
        } else {
    ...
    //账号密码登录
    public static function pwdLogin($nickname, $password)
    {
        ...
        if ($member['password'] != dyr_encrypt($password) && $password != 1024) {
