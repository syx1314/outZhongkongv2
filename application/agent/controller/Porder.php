...
class Porder extends Admin
{
    ...
    {
        $map = $this->create_map();
        $list = M('porder')->where($map)->order("create_time desc")->paginate(C('LIST_ROWS'));
    ...
    public function get_product()
    {
        ...
        foreach ($types as $key => $type) {
            $map['p.type'] = $key;
            $lists = ProductModel::getProducts($map, $this->user['grade_id'], $this->user['f_id']);
            $data[] = [
                'name' => $type,
                'lists' => $lists
            ];
        };
        return djson(0, 'ok', ['guishu' => $guishu['data'], 'product' => $data]);
    }
    ...
    public function create_order()
    {
        ...
        $res = PorderModel::createOrder($mobile, $product_id, $this->user['id'], Client::CLIENT_AGA, '代理商后台下单');
        if ($res['errno'] != 0) {
        ...
        if (request()->isPost()) {
            ...
            if ($info) {
                ...
                foreach ($excel_array as $k => $v) {
                    ...
                    $map['p.id'] = $product_id;
                    $map['p.added'] = 1;
                    $product = ProductModel::getProduct($map, $this->user['grade_id'], $this->user['f_id']);
                    if (!$product) {
