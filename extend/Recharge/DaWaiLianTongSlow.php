...
class DaWaiLianTongSlow
{
    ...
    public function recharge($out_trade_num, $mobile, $param, $isp = '')
    {
        ...
        $signStr=$data['mchid'].$data['tel'].$data['price'].$data['orderid'].$data['teltype']
            .$data['timeout'].$data['notify'].$data['time'].$data['rand'].$this->apikey;
    ...
    private function http_post($url, $param)
    {
        ...
        if (intval($aStatus["http_code"]) == 0) {
            ...
            if (intval($aStatus["http_code"]) == 200) {
                ...
                if ($result['code'] == 0) {
                    return rjson(0, $result['code'], $result['msg']);
                } else {
                    return rjson(1, $result['code'] . $result['msg'], $result['msg']);
                }
            } else {
